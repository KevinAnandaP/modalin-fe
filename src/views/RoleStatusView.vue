...
<template>
  <AuthLayout>
    <div class="bg-white p-8 sm:p-10 rounded-xl border border-[#0F6E56]/40 shadow-xs w-full max-w-xl mx-auto font-inter my-8 text-center">
      ...
      <div v-else>
        ...
        <div v-if="statusState === 'approved'" class="space-y-6">
          <div class="w-16 h-16 bg-[#E1F5EE] text-[#0F6E56] rounded-full flex items-center justify-center mx-auto">
            <CheckCircle2 class="w-10 h-10" />
          </div>
          <div>
            <span class="inline-block px-3 py-1 bg-[#E1F5EE] text-[#0F6E56] text-xs font-semibold rounded-full mb-3">
              Peran Disetujui (Approved)
            </span>
            <h2 class="text-2xl font-semibold text-[#1F2937] font-newsreader">
              Status Peran Akun Anda Aktif
            </h2>
            <p class="text-sm text-[#52605D] mt-2">
              Akun Anda telah disetujui untuk peran: <span class="font-semibold text-[#1F2937] capitalize">{{ roles.join(', ') }}</span>.
            </p>
          </div>
          ...
          <div class="space-y-3 pt-2 text-left">
            <div v-if="roles.includes('borrower')" class="p-4 bg-[#F9FAFB] rounded-lg border border-gray-200 flex items-center justify-between">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <h4 class="font-semibold text-sm text-[#1F2937]">Peran Peminjam (UMKM)</h4>
                  <p class="text-xs text-[#52605D]">Buat profil usaha & ajukan modal usaha/rintisan</p>
                </div>
              </div>
              <button
                @click="handleNextAction('borrower')"
                class="px-3.5 py-2 bg-[#0F6E56] text-white text-xs font-semibold rounded-lg hover:bg-[#0A5744] transition-colors cursor-pointer"
              >
                Isi Profil Bisnis
              </button>
            </div>
            ...
            <div v-if="roles.includes('lender')" class="p-4 bg-[#F9FAFB] rounded-lg border border-gray-200 flex items-center justify-between">
              <div class="flex items-center gap-3">
                <Coins class="w-5 h-5 text-[#0F6E56]" />
                <div>
                  <h4 class="font-semibold text-sm text-[#1F2937]">Peran Pemberi Modal</h4>
            ...
            <div v-if="roles.includes('verifier')" class="p-4 bg-[#F9FAFB] rounded-lg border border-gray-200 flex items-center justify-between">
              <div class="flex items-center gap-3">
                <ShieldCheck class="w-5 h-5 text-[#0F6E56]" />
                <div>
                  <h4 class="font-semibold text-sm text-[#1F2937]">Peran Verifikator</h4>
          ...
          <div class="pt-3 border-t border-gray-100 flex items-center justify-between text-xs text-[#52605D]">
            ...
            <button @click="router.push('/request-role')" class="text-[#0F6E56] font-semibold hover:underline cursor-pointer">
              + Ajukan Peran Lain
            </button>
          </div>
        </div>
        ...
        <div v-else-if="statusState === 'pending'" class="space-y-6">
          <div class="w-16 h-16 bg-[#FEF3C7] text-[#B45309] rounded-full flex items-center justify-center mx-auto">
            <Clock class="w-10 h-10 animate-pulse" />
          </div>
          <div>
            <span class="inline-block px-3 py-1 bg-[#FEF3C7] text-[#B45309] text-xs font-semibold rounded-full mb-3">
          ...
          <div class="p-4 bg-[#F9FAFB] rounded-lg border border-gray-200 text-left text-xs text-[#52605D] space-y-1">
            <span class="font-semibold text-[#1F2937] block">Tahapan Alur Pengajuan:</span>
            <p>1. Pendaftaran Akun (Selesai)</p>
            <p class="text-[#B45309] font-medium">2. Pengajuan Role (Sedang Ditinjau Admin)</p>
          ...
          <div class="pt-2 flex flex-col gap-3">
            <button
              @click="fetchStatus"
              class="w-full py-3 bg-[#0F6E56] hover:bg-[#0A5744] text-white font-semibold text-sm rounded-lg transition-colors cursor-pointer shadow-xs flex items-center justify-center gap-2"
            >
            ...
            <button
              ...
              class="w-full py-2.5 border border-gray-300 text-[#52605D] hover:text-[#1F2937] font-medium text-sm rounded-lg transition-colors cursor-pointer"
            >
              Keluar Akun
            </button>
          </div>
