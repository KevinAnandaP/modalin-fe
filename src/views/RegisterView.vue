...
const handleRegister = async () => {
  ...
  try {
    ...
    successMessage.value = 'Pendaftaran berhasil! Mengalihkan...'
...
const triggerGoogleOAuth = () => {
  ...
  const redirectUri = import.meta.env.VITE_GOOGLE_REDIRECT_URI || `${window.location.origin}/register`
...
<template>
  <AuthLayout>
    <div class="bg-white p-5 sm:p-8 lg:p-10 rounded-xl border border-[#0F6E56]/40 shadow-xs w-full max-w-lg mx-auto font-inter my-4 sm:my-8">
      ...
      <div v-if="successMessage" class="mb-5 p-3.5 bg-emerald-50 border border-emerald-200 text-[#0F6E56] text-sm rounded-lg flex items-center gap-2 text-left">
      ...
      <form @submit.prevent="handleRegister" class="space-y-4">
        ...
        <div class="relative flex py-2 items-center">
          ...
          <span class="flex-shrink mx-4 text-xs text-[#52605D]">atau isi formulir</span>
          <div class="flex-grow border-t border-gray-200"></div>
        </div>
